...
// KojinStudios Bots — Stripe Webhook Handler
...
async function handleSubscriptionUpdated(subscription) {
    ...
    const embed = {
        ...
        fields: [
            ...
            { name: 'Amount', value: `$${(subscription.items.data[0]?.price.unit_amount / 100).toFixed(2)}/${subscription.items.data[0]?.price.recurring.interval}`, inline: true }
        ],
        timestamp: new Date().toISOString(),
        footer: { text: 'KojinStudios Bots — Subscription System' }
    };
...
async function updateBotDatabase(guildId, botName, session, action) {
    try {
        const botApiUrl = process.env.BOT_API_URL || 'http://localhost:5000/api/update-subscription';
        const payload = {
            server_id: guildId,
            bot_name: botName,
            action: action,
